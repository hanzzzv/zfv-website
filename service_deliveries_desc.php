...
<div class="wrapper">
    ...
    <div class="main-container">
        ...
        $currentDescription6 = "";
        try {
            $stmt = $conn->prepare("SELECT description6 FROM services6 WHERE id6 = 1 LIMIT 1");
            $stmt->execute();
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($row && isset($row['description5'])) {
                $currentDescription6 = $row['description6'];
            }
        } catch(PDOException $e) {
        ...
        $descriptionExists6 = !empty($currentDescription6);
        ...
        if (isset($_POST['save'])) {
            ...
            try {
                if ($descriptionExists6) {
                    // Update existing description
                    $stmt = $conn->prepare("UPDATE services6 SET description6 = ? WHERE id6 = 1");
                    $stmt->execute([$description6]);
                    echo "Service description updated successfully.";
                } else {
                    // Add new description
                    $stmt = $conn->prepare("INSERT INTO services6 (id6, description6) VALUES (1, ?)");
                    $stmt->execute([$description6]);
                    echo "Service description added successfully.";
                    $descriptionExists6 = true;
                }
                // Update the current description after adding or updating
                $currentDescription6 = $description6;
            } catch(PDOException $e) {
                echo "Error: " . $e->getMessage();
            }
        }
        ...
        <div class="container">
            ...
            <form action="" method="POST">
                <div class="form-group">
                    <textarea id="custom-description6" class="form-control" rows="4" name="description6" required><?php echo $currentDescription6; ?></textarea>
                </div>
                <div class="btn-container">
...
<script>
    function addBold() {
        var textarea = document.getElementById('custom-description6');
        textarea.value += '<b></b>';
    }
