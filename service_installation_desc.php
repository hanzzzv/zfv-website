...
<div class="wrapper">
    ...
    <div class="main-container">
        ...
        $currentDescription5 = "";
        try {
            $stmt = $conn->prepare("SELECT description5 FROM services5 WHERE id5 = 1 LIMIT 1");
            $stmt->execute();
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($row && isset($row['description5'])) {
                $currentDescription5 = $row['description5'];
            }
        } catch(PDOException $e) {
        ...
        $descriptionExists5 = !empty($currentDescription5);
        ...
        if (isset($_POST['save'])) {
            ...
            try {
                if ($descriptionExists5) {
                    // Update existing description
                    $stmt = $conn->prepare("UPDATE services5 SET description5 = ? WHERE id5 = 1");
                    $stmt->execute([$description5]);
                    echo "Service description updated successfully.";
                } else {
                    // Add new description
                    $stmt = $conn->prepare("INSERT INTO services5 (id5, description5) VALUES (1, ?)");
                    $stmt->execute([$description5]);
                    echo "Service description added successfully.";
                    $descriptionExists5 = true;
                }
                // Update the current description after adding or updating
                $currentDescription5 = $description5;
            } catch(PDOException $e) {
                echo "Error: " . $e->getMessage();
            }
        }
        ...
        <div class="container">
            ...
            <h2>Installation Description</h2>
            ...
            <form action="" method="POST">
                <div class="form-group">
                    <textarea id="custom-description5" class="form-control" rows="4" name="description5" required><?php echo $currentDescription5; ?></textarea>
                </div>
                <div class="btn-container">
...
<script>
    function addBold() {
        var textarea = document.getElementById('custom-description5');
        textarea.value += '<b></b>';
    }
