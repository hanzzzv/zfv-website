...
<div class="wrapper">
    ...
    <div class="main-container">
        ...
        $currentDescription4 = "";
        try {
            $stmt = $conn->prepare("SELECT description4 FROM services4 WHERE id4 = 1 LIMIT 1");
            $stmt->execute();
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($row && isset($row['description4'])) {
                $currentDescription4 = $row['description4'];
            }
        } catch(PDOException $e) {
        ...
        $descriptionExists4 = !empty($currentDescription4);
        ...
        if (isset($_POST['save'])) {
            ...
            try {
                if ($descriptionExists4) {
                    // Update existing description
                    $stmt = $conn->prepare("UPDATE services4 SET description4 = ? WHERE id4 = 1");
                    $stmt->execute([$description4]);
                    echo "Service description updated successfully.";
                } else {
                    // Add new description
                    $stmt = $conn->prepare("INSERT INTO services4 (id4, description4) VALUES (1, ?)");
                    $stmt->execute([$description4]);
                    echo "Service description added successfully.";
                    $descriptionExists4 = true;
                }
                // Update the current description after adding or updating
                $currentDescription4 = $description4;
            } catch(PDOException $e) {
                echo "Error: " . $e->getMessage();
            }
        }
        ...
        <div class="container">
            ...
            <form action="" method="POST">
                <div class="form-group">
                    <textarea id="custom-description4" class="form-control" rows="4" name="description4" required><?php echo $currentDescription4; ?></textarea>
                </div>
                <div class="btn-container">
...
<script>
    function addBold() {
        var textarea = document.getElementById('custom-description4');
        textarea.value += '<b></b>';
    }
