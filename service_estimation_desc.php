...
<div class="wrapper">
    ...
    <div class="main-container">
        ...
        $currentDescription3 = "";
        try {
            $stmt = $conn->prepare("SELECT description3 FROM services3 WHERE id3 = 1 LIMIT 1");
            $stmt->execute();
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($row && isset($row['description3'])) {
                $currentDescription3 = $row['description3'];
            }
        } catch(PDOException $e) {
        ...
        $descriptionExists3 = !empty($currentDescription3);
        ...
        if (isset($_POST['save'])) {
            ...
            try {
                if ($descriptionExists3) {
                    // Update existing description
                    $stmt = $conn->prepare("UPDATE services3 SET description3 = ? WHERE id3 = 1");
                    $stmt->execute([$description3]);
                    echo "Service description updated successfully.";
                } else {
                    // Add new description
                    $stmt = $conn->prepare("INSERT INTO services3 (id3, description3) VALUES (1, ?)");
                    $stmt->execute([$description3]);
                    echo "Service description added successfully.";
                    $descriptionExists3 = true;
                }
                // Update the current description after adding or updating
                $currentDescription3 = $description3;
            } catch(PDOException $e) {
                echo "Error: " . $e->getMessage();
            }
        }
        ...
        <div class="container">
            ...
            <form action="" method="POST">
                <div class="form-group">
                    <textarea id="custom-description3" class="form-control" rows="4" name="description3" required><?php echo $currentDescription3; ?></textarea>
                </div>
                <div class="btn-container">
...
<script>
    function addBold() {
        var textarea = document.getElementById('custom-description3');
        textarea.value += '<b></b>';
    }
