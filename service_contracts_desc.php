...
<div class="wrapper">
    ...
    <div class="main-container">
        ...
        $currentDescription2 = "";
        try {
            $stmt = $conn->prepare("SELECT description2 FROM services2 WHERE id2 = 1 LIMIT 1");
            $stmt->execute();
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($row && isset($row['description2'])) {
                $currentDescription2 = $row['description2'];
            }
        } catch(PDOException $e) {
        ...
        $descriptionExists2 = !empty($currentDescription2);
        ...
        if (isset($_POST['save'])) {
            ...
            try {
                if ($descriptionExists2) {
                    // Update existing description
                    $stmt = $conn->prepare("UPDATE services2 SET description2 = ? WHERE id2 = 1");
                    $stmt->execute([$description2]);
                    echo "Service description updated successfully.";
                } else {
                    // Add new description
                    $stmt = $conn->prepare("INSERT INTO services2 (id2, description2) VALUES (1, ?)");
                    $stmt->execute([$description2]);
                    echo "Service description added successfully.";
                    $descriptionExists2 = true;
                }
                // Update the current description after adding or updating
                $currentDescription2 = $description2;
            } catch(PDOException $e) {
                echo "Error: " . $e->getMessage();
            }
        }
        ...
        <div class="container">
            ...
            <form action="" method="POST">
                <div class="form-group">
                    <textarea id="custom-description2" class="form-control" rows="4" name="description2" required><?php echo $currentDescription2; ?></textarea>
                </div>
                <div class="btn-container">
...
<script>
    function addBold() {
        var textarea = document.getElementById('custom-description2');
        textarea.value += '<b></b>';
    }
